...
try:
    ...
    from generation import generate_documents_word, combined_generation_process
...
try:
    ...
    log_and_show_error(ImportError, "Не вдалося імпортувати модуль win32com.client.\nУстановіть його командою: pip install pywin32", None)
...
#main_app_root = SafeCTk()
#main_app_root.protocol("WM_DELETE_WINDOW", on_app_close)
...
from auth_utils import ask_password
...
if __name__ == "__main__":
    try:
        ...
        ask_password(on_success_callback=on_success)
        ...
        input("Натисніть Enter для завершення...")  # <--- добавлено: не даёт консоли закрыться сразу
        sys.exit(1)
