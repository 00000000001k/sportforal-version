...
def generate_documents_word():
    ...
    save_dir = messagebox.filedialog.askdirectory(title="Оберіть папку для збереження документів Word")
    if not save_dir:
        return False
    ...
    try:
        ...
        for block in document_blocks:
            ...
            try:
                ...
                safe_name = "".join(c if c.isalnum() or c in " -" else "_" for c in товар_name)[:50]
        ...
        log_and_show_error(type(e_main_word), f"Загальна помилка при генерації документів Word: {e_main_word}", sys.exc_info()[2])
        return False
    finally:
        if word_app:
            try:
...
def combined_generation_process():
    ...
    if not generate_documents_word():
