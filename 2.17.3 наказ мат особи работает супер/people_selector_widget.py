...
class PeopleSelectorDialog:
    ...
    def _get_people_in_special_roles(self):
        """Повертає множину людей, які вже обрані в спеціальних ролях"""
        people_in_roles = set()
        for role_id in SPECIAL_ROLES.keys():
            selected_person = people_manager.get_special_role(role_id)
            if selected_person:
                people_in_roles.add(selected_person)
        return people_in_roles
    ...
    def _create_main_people_section(self, parent):
        ...
        # Отримуємо людей, які вже в спеціальних ролях
        people_in_special_roles = self._get_people_in_special_roles()

        for person_id, person_data in PEOPLE_CONFIG.items():
            # Пропускаємо людей, які вже обрані в спеціальних ролях
            if person_id in people_in_special_roles:
                continue
    ...
    def _on_special_role_change(self, role_id, person_id):
        ...
        # Якщо людина була обрана в основних особах, видаляємо її звідти
        if people_manager.is_person_selected(person_id):
            people_manager.toggle_person(person_id)
