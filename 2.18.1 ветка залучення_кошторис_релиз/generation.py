...
def generate_documents_word(tabview):
    ...
    try:
        ...
        # === ДОБАВЛЯЕМ ГЕНЕРАЦИЮ КОШТОРИСА ===
        koshtorys_generated = False
        try:
            print("[INFO] Розпочинаємо генерацію кошторису...")
            # Импортируем функцию fill_koshtorys из модуля koshtorys
            from koshtorys import fill_koshtorys
        ...
        # Показываем результат с учетом кошториса
        if generated_count > 0:
            if koshtorys_generated:
                messagebox.showinfo("Успіх",
                                    f"{generated_count} документ(и) Word та кошторис збережено успішно в папці:\n{save_dir}")
            else:
                messagebox.showinfo("Частковий успіх",
                                    f"{generated_count} документ(и) Word збережено успішно в папці:\n{save_dir}\n\n"
                                    "Увага: Виникла помилка при генерації кошторису.")
        else:
